...
package com.huawei.hms.location.backend.utils;
...
import com.huawei.hms.location.backend.interfaces.Mapper;
...
import static com.huawei.hms.location.backend.helpers.Exceptions.ERR_NO_FUSED_LOCATION_PROVIDER;
import static com.huawei.hms.location.backend.helpers.Exceptions.ERR_NO_PERMISSION;
import static com.huawei.hms.location.backend.utils.PlatformUtils.mapperWrapper;


public class LocationUtils {
    private static final String TAG = LocationUtils.class.getSimpleName();

    public static final Mapper<JSONObject, LocationRequest> FROM_JSON_OBJECT_TO_LOCATION_REQUEST = mapperWrapper((JSONObject readableMap) -> LocationRequest.create()
    ...
    public static final Mapper<JSONObject, LocationSettingsRequest> FROM_JSON_OBJECT_TO_LOCATION_SETTINGS_REQUEST = mapperWrapper((JSONObject locationRequestMap) -> {
            JSONArray locationRequestsArray = locationRequestMap.getJSONArray("locationRequests");
            List<LocationRequest> locationRequestList = PlatformUtils.mapJSONArray(locationRequestsArray, FROM_JSON_OBJECT_TO_LOCATION_REQUEST);
    ...
    public static final Mapper<Location, JSONObject> FROM_LOCATION_TO_JSON_OBJECT = mapperWrapper((Location location) -> {
    ...
    public static final Mapper<HWLocation, JSONObject> FROM_HW_LOCATION_TO_JSON_OBJECT = mapperWrapper((HWLocation hwLocation) -> {
    ...
    public static final Mapper<LocationSettingsStates, JSONObject> FROM_LOCATION_SETTINGS_STATES_TO_JSON_OBJECT = mapperWrapper((LocationSettingsStates locationSettingsStates) -> {
        JSONObject result = new JSONObject();
    ...
    public static final Mapper<LocationAvailability, JSONObject> FROM_LOCATION_AVAILABILITY_TO_JSON_OBJECT = mapperWrapper((LocationAvailability locationAvailability) -> {
        JSONObject result = new JSONObject();
    ...
    public static final Mapper<LocationSettingsResponse, JSONObject> FROM_LOCATION_SETTINGS_STATES_RESPONSE_TO_JSON_OBJECT = mapperWrapper((LocationSettingsResponse locationSettingsResponse) -> {
        JSONObject result = new JSONObject();
        result.put("locationSettingsStates", FROM_LOCATION_SETTINGS_STATES_TO_JSON_OBJECT.map(locationSettingsResponse.getLocationSettingsStates()));
        return result;
    });

    public static boolean checkForObstacles(HMSProvider provider, FusedLocationProviderClient fused, final HMSCallback callback) {
